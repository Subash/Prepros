...
prepros.factory('markdown', function (config, utils, notification) {
    ...
    var format = function (filePath, projectPath) {
        ...
        var shortOutput = filePath.replace(/\.markdown|\.md/gi, config.user.htmlExtension).replace(/\\/g, '/');

        //Show Relative path if output file is within project folder
        if (path.relative(projectPath, filePath).indexOf('.\\') === -1) {

            shortOutput = path.relative(projectPath, output).replace(/\.markdown|\.md/gi, config.user.htmlExtension).replace(/\\/g, '/');
        }
